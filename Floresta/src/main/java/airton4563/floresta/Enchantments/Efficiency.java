...
public class Efficiency {
    ...
    public static ItemStack getEnchantedBook(Player player) {
        ...
        lore.add("§7Aumente a velocidade");
        ...
        ItemStack item = Util.createItem(Material.ENCHANTED_BOOK, "§aEficiência", lore);
        item.getItemMeta().getItemFlags().add(ItemFlag.HIDE_ENCHANTS);
        return item;
    }

    public static void load() {
        FileConfiguration config = GetYML.getConfiguration("encantamentos/Eficiencia.yml");
        ...
        if (enchantmentsSection != null) {
            for (String levelString : enchantmentsSection.getKeys(false)) {
                ...
                int lenhas = GetYML.getInt("Leveis." + levelString, "encantamentos/Eficiencia.yml");
